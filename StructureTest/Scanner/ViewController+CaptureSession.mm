...
- (bool)videoDeviceSupportsHighResColor
{
    ...
    {
        AVFrameRateRange* firstFrameRateRange =
            ((AVFrameRateRange*)[format.videoSupportedFrameRateRanges objectAtIndex:0]);
        ...
        if ((formatMaxFps < 15) // Max framerate too low.
            ...
            || (formatMaxFps == 24 &&
                formatMinFps > 15) // We can neither do the 24 FPS max framerate, nor fall back to 15.
        )
            continue;
...
- (void)setupCaptureSession
{
    ...
    if (_captureSession == nil)
    {
      ...
      const bool useUnifiedSession = [defaults boolForKey:@"beta_capture_session"];
      if(useUnifiedSession)
        _captureSession = [STCaptureSession newCaptureSessionBeta];
      else
    ...
    STCaptureSessionDepthFrameResolution depthResolution;
    switch (_dynamicOptions.depthResolution)
    {
        case DepthResolution_Qvga: depthResolution = STCaptureSessionDepthFrameResolution320x240; break;
        case DepthResolution_Vga: depthResolution = STCaptureSessionDepthFrameResolution640x480; break;
        case DepthResolution_Full: depthResolution = STCaptureSessionDepthFrameResolution1280x960; break;
    }

    NSDictionary* sensorConfig = @{
        kSTCaptureSessionOptionColorResolutionKey: @(resolution),
        kSTCaptureSessionOptionDepthFrameResolutionKey: @(depthResolution),
        ...
        kSTCaptureSessionOptionDepthSearchWindowKey:
            @[[NSNumber numberWithFloat:_depthWindowSearchWidth], [NSNumber numberWithFloat:_depthWindowSearchHeight]],
    };
    ...
    sensorChargerStateChanged:(STCaptureSessionSensorChargerState)chargerState
{
    switch (chargerState)
    {
        ...
        default:
            @throw [NSException exceptionWithName:@"Scanner" reason:@"Unknown STCaptureSessionSensorChargerState!"
                                         userInfo:nil];
            break;
    }
    [self updateViewsWithSensorStatus];
                  ...
                  type:(STCaptureSessionSampleType)type
{
    switch (type)
    {
        ...
        case STCaptureSessionSampleTypeUnknown:
            @throw [NSException exceptionWithName:@"Scanner" reason:@"Unknown STCaptureSessionSampleType!"
                                         userInfo:nil];
            break;
        default:
...
- (void)captureSession:(STCaptureSession*)captureSession onLensDetectorOutput:(STDetectedLensStatus)detectedLensStatus
{
    switch (detectedLensStatus)
    {
        ...
        default:
            @throw [NSException exceptionWithName:@"Scanner" reason:@"Unknown STDetectedLensStatus!" userInfo:nil];
            break;
    }
}
