...
MeshRenderer::MeshRenderer() : d(new PrivateData)
{}
...
void MeshRenderer::uploadMesh(STMesh* mesh)
{
    ...
    for (int meshIndex = 0; meshIndex < numUploads; ++meshIndex)
    {
        ...
        if (d->hasPerVertexNormals)
        {
            ...
            glBufferData(
                GL_ARRAY_BUFFER,
                numVertices * sizeof(GLKVector3),
                [mesh meshPerVertexNormals:meshIndex],
                GL_STATIC_DRAW);
        }
        ...
        if (d->hasPerVertexColor)
        {
            ...
            glBufferData(
                GL_ARRAY_BUFFER,
                numVertices * sizeof(GLKVector3),
                [mesh meshPerVertexColors:meshIndex],
                GL_STATIC_DRAW);
        }
        ...
        if (d->hasPerVertexUV)
        {
            ...
            glBufferData(
                GL_ARRAY_BUFFER,
                numVertices * sizeof(GLKVector2),
                [mesh meshPerVertexUVTextureCoords:meshIndex],
                GL_STATIC_DRAW);
        }
        ...
        glBufferData(
            ...
            [mesh numberOfMeshFaces:meshIndex] * sizeof(unsigned int) * 3,
            [mesh meshFaces:meshIndex],
            GL_STATIC_DRAW);

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, d->linesVbo[meshIndex]);
        glBufferData(
            GL_ELEMENT_ARRAY_BUFFER,
            [mesh numberOfMeshLines:meshIndex] * sizeof(unsigned int) * 2,
            [mesh meshLines:meshIndex],
            GL_STATIC_DRAW);
...
void MeshRenderer::renderPartialMesh(int meshIndex)
{
    ...
    switch (d->currentRenderingMode)
    {
        ...
        default: NSLog(@"Unknown rendering mode."); break;
    }
...
{
    if (d->currentRenderingMode == RenderingModePerVertexColor && !d->hasPerVertexColor && d->hasTexture &&
        d->hasPerVertexUV)
    {
        NSLog(@"Warning: The mesh has no per-vertex colors, but a texture, switching the rendering mode to "
              @"RenderingModeTextured");
        d->currentRenderingMode = RenderingModeTextured;
    }
    else if (
        d->currentRenderingMode == RenderingModeTextured && (!d->hasTexture || !d->hasPerVertexUV) &&
        d->hasPerVertexColor)
    {
        NSLog(@"Warning: The mesh has no texture, but per-vertex colors, switching the rendering mode to "
    ...
    switch (d->currentRenderingMode)
    {
        ...
        default: NSLog(@"Unknown rendering mode."); return;
    }
