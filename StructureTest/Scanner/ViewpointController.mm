...
ViewpointController::ViewpointController(float screenSizeX, float screenSizeY) : d(new PrivateData)
{
...
{
    d->meshCenterOnScreen = GLKVector2Add(
        GLKVector2Subtract(touch, d->twoFingersPanWhenGestureBegan),
        d->meshCenterOnScreenWhenPanGestureBegan);
    d->cameraOrProjectionChangedSinceLastUpdate = true;
}
...
bool ViewpointController::update()
{
    ...
    if (GLKVector2Length(d->modelViewRotationVelocity) > 1e-5f)
    {
        ...
        if (std::abs(d->modelViewRotationVelocity.x) < 1.f)
            d->modelViewRotationVelocity.x = 0;
        if (std::abs(d->modelViewRotationVelocity.y) < 1.f)
...
GLKMatrix4 ViewpointController::currentProjectionCenterTranslation() const
{
    ...
    return GLKMatrix4MakeTranslation(
        -deltaFromScreenCenter.x / d->screenCenter.x,
        deltaFromScreenCenter.y / d->screenCenter.y,
        0);
}
