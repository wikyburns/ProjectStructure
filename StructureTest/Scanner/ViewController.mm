...
{
    NSTimer* _batteryLevelCheckTimer;
}

@property(nonatomic) BatteryLevelState lastBatteryLevelState;
...
- (void)viewDidLoad
{
    ...
    _alignCubeWithCamera = [defaults boolForKey:@"alignCubeWithCamera"];
    _fixedCubePosition = [defaults boolForKey:@"fixedCubePostion"];

    _enableDepthWindowOverride = [defaults boolForKey:@"enableWindowOverride"];
    _runDepthRefinement = [defaults boolForKey:@"enableDepthRefinement"];

    _enableDepthWindowOverride = [defaults boolForKey:@"enableWindowOverride"];
    _runDepthRefinement = [defaults boolForKey:@"enableDepthRefinement"];


    _fixedCubeDistanceSwitch.on = _fixedCubePosition;
    _fixedCubeDistanceSwitch.hidden = false;

    _alignCubeWithCameraSwitch.on = !_alignCubeWithCamera;


    if (_enableDepthWindowOverride)
    {
        _depthWindowSearchWidth = [defaults floatForKey:@"depthWindowSearchWidth"];
        _depthWindowSearchHeight = [defaults floatForKey:@"depthWindowSearchWidth"];
    }
    else
    {
        // use default values
        _depthWindowSearchWidth = 15;
        _depthWindowSearchHeight = 11;
    }
    ...
    self.lastBatteryLevelState = BatteryLevelState::Full;
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self updateViewsWithSensorStatus];
...
- (void)presentMeshViewer:(STMesh*)mesh
{
    ...
    if (self->_timeTagOnOcc)
    {
        NSError* error;

        // Request a zipped OBJ file, potentially with embedded MTL and texture.
        NSDictionary* options = @{
            kSTMeshWriteOptionFileFormatKey: @(STMeshWriteOptionFileFormatPlyFile),
            kSTMeshWriteOptionUseXRightYUpConventionKey: @YES
        };

        STMesh* meshToSend = _meshViewController.mesh;
        NSString* meshOutput = @"[AppDocuments]/Scanner-";
        meshOutput = [meshOutput stringByAppendingString:self->_timeTagOnOcc];
        meshOutput = [meshOutput stringByAppendingString:@".ply"];
        if (![meshToSend writeToFile:meshOutput options:options error:&error])
        {
            @throw [NSException exceptionWithName:@"BadMesh" reason:[error localizedDescription] userInfo:nil];
        }
        self->_timeTagOnOcc = nullptr;
    }
...
- (void)enterCubePlacementState
{
    ...
    self.fixedCubeDistanceSwitch.hidden = NO;
    self.fixedCubeDistanceLabel.hidden = self.fixedCubeDistanceSwitch.hidden;
    self.alignCubeWithCameraSwitch.hidden = NO;
    self.alignCubeWithCameraLabel.hidden = self.alignCubeWithCameraSwitch.hidden;
    self.boxSizeLabel.hidden = NO;

    self.cubeDistanceSlider.hidden = !_fixedCubePosition;

    self.boxDistanceLabel.hidden = !_fixedCubePosition;
    self.boxDistanceLabel.text = [NSString stringWithFormat:@"Distance %1.2f m", self->_options.cubeDistanceValue];

    self.alignCubeWithCameraSwitch.hidden = NO;
    self.boxSizeLabel.text = [NSString
        stringWithFormat:@"Size %1.2f m", self->_options.initVolumeSizeInMeters.x * _volumeScale.currentScale];
...
- (void)enterScanningState
{
    ...
    self.fixedCubeDistanceSwitch.hidden = YES;
    self.cubeDistanceSlider.hidden = YES;
    self.boxDistanceLabel.hidden = YES;
    self.boxSizeLabel.hidden = YES;
    self.alignCubeWithCameraSwitch.hidden = YES;
    self.fixedCubeDistanceLabel.hidden = self.fixedCubeDistanceSwitch.hidden;
    self.alignCubeWithCameraLabel.hidden = self.alignCubeWithCameraSwitch.hidden;
    ...
    if (_fixedCubePosition)
    {
        ...
        {
            _slamState.lssTracker.initialCameraPose = _slamState.cameraPose;
        }
        else
        {
            _slamState.tracker.initialCameraPose = _slamState.cameraPose;
        }
    }
    else
    {
        if (_slamState.slamOption == SlamOption::STSlamManager)
        {
            _slamState.lssTracker.initialCameraPose = _slamState.initialDepthCameraPose;
        }
        else
        {
...
- (void)enterViewingState
{
    ...
    if (_slamState.slamOption == SlamOption::STSlamManager)
        [_slamState.lssTracker finalizeTriangleMesh];
    else
...
- (BOOL)currentStateNeedsSensor
{
    switch (_slamState.scannerState)
    {
        ...
        case ScannerStateScanning: return TRUE;
...
- (void)processDeviceMotion:(CMDeviceMotion*)motion withError:(NSError*)error
{
    ...
    if (_slamState.scannerState == ScannerStateCubePlacement || _slamState.scannerState == ScannerStateScanning)
    {
        ...
        if (_slamState.slamOption == SlamOption::STSlamManager)
            [_slamState.lssTracker updateCameraPoseWithMotion:motion];
        else
             ...
             depthStreamPresetMode:(STCaptureSessionPreset)depthStreamPresetMode
{
    ...
    _dynamicOptions.depthResolution = depthResolution;
    [self setupCaptureSession];
                   ...
                   irAnalogGainValue:(STCaptureSessionSensorAnalogGainMode)irAnalogGainValue
{
    _captureSession.properties = @{
        kSTCaptureSessionPropertySensorIRExposureModeKey:
            @(irAutoExposureEnabled ? STCaptureSessionSensorExposureModeAuto
...
{
    _dynamicOptions.stSlamManagerIsSelected = stSlamManagerSelected;
    [self onSLAMOptionsChanged];
}
...
- (void)adjustVolumeSize:(GLKVector3)volumeSize
{
    ...
    self.boxSizeLabel.text = [NSString stringWithFormat:@"Size %1.2f m", volumeSize.x];
...
- (IBAction)alignCubeWithCameraDidChange:(UISwitch*)sender
{
    _alignCubeWithCamera = !sender.isOn;
    [NSUserDefaults.standardUserDefaults setBool:sender.isOn forKey:@"alignCubeWithCamera"];
    [self onSLAMOptionsChanged];
}

- (IBAction)fixedCubePositionDidChange:(UISwitch*)sender
{
    _fixedCubePosition = sender.isOn;
    self.boxDistanceLabel.hidden = !_fixedCubePosition;
    // self.cubeDistanceSlider.hidden = !_fixedCubePosition;
    [NSUserDefaults.standardUserDefaults setBool:sender.isOn forKey:@"fixedCubePostion"];
    [self onSLAMOptionsChanged];
}
...
{
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    const bool recordOcc = [defaults boolForKey:@"recordOcc"];
    ...
    if (recordOcc)
    {
        ...
        self->_timeTagOnOcc = [formatter stringFromDate:currentDate];

        NSString* occString = @"[AppDocuments]/Scanner-SN";
        occString = [occString stringByAppendingString:self->_captureSession.sensorSerialNumber];
        occString = [occString stringByAppendingString:@"_"];
        occString = [occString stringByAppendingString:self->_timeTagOnOcc];
        occString = [occString stringByAppendingString:@".occ"];


        bool success = [_captureSession.occWriter startWriting:occString appendDateAndExtension:NO];
        if (!success)
        {
...
- (IBAction)doneButtonPressed:(id)sender
{
    if (_captureSession.occWriter.isWriting)
    {
        ...
        if (!success)
        {
            @throw [NSException exceptionWithName:@"Scanner" reason:@"Could not properly stop OCC writer."
                                         userInfo:nil];
        }
    }
...
- (void)showSensorRelatedViews
{
    self.batteryView.hidden = NO;
    _settingsPopupView.hidden = NO;
    self.alignCubeWithCameraLabel.hidden = NO;
    self.fixedCubeDistanceLabel.hidden = NO;
    self.boxDistanceLabel.hidden = NO;
    self.boxSizeLabel.hidden = NO;
    self.alignCubeWithCameraSwitch.hidden = NO;
    self.fixedCubeDistanceSwitch.hidden = NO;
    self.scanButton.hidden = NO;
    self.firmwareUpdateView.hidden = NO;
}

- (void)hideSensorRelatedViews
{
    self.batteryView.hidden = YES;
    _settingsPopupView.hidden = YES;
    self.alignCubeWithCameraLabel.hidden = YES;
    self.fixedCubeDistanceLabel.hidden = YES;
    self.boxDistanceLabel.hidden = YES;
    self.boxSizeLabel.hidden = YES;
    self.alignCubeWithCameraSwitch.hidden = YES;
    self.fixedCubeDistanceSwitch.hidden = YES;
    self.scanButton.hidden = NO;
    self.firmwareUpdateView.hidden = YES;
}
...
- (void)showAppStatusMessage:(NSString*)msg
{
    ...
    [UIView animateWithDuration:0.5f animations:^{ self.appStatusMessageLabel.alpha = 1.0f; } completion:nil];
}
...
- (void)hideAppStatusMessage
{
    ...
    [UIView animateWithDuration:0.5f animations:^{ weakSelf.appStatusMessageLabel.alpha = 0.0f; }
    ...
    _batteryLevelCheckTimer = [NSTimer scheduledTimerWithTimeInterval:10.0f target:self
                                                             selector:@selector(batteryLevelCheckTick:)
                                                             userInfo:nil
                                                              repeats:YES];
    [_batteryLevelCheckTimer fire];
}

- (void)updateViewsWithSensorStatus
{
    ...
    // Sensor related views are shown only while sensor is ready
    if (_captureSession.sensorMode == STCaptureSessionSensorModeReady) {
        [self showSensorRelatedViews];
    } else {
        [self hideSensorRelatedViews];
    }
    ...
    if (needToRunCalibrator)
    {
        ...
        switch (_captureSession.calibrationType)
        {
            ...
            case STCalibrationTypeDeviceSpecific:
                ...
                break;
            default: NSLog(@"WARNING: Unknown calibration type returned from the capture session."); break;
        }
        ...
        [_calibrationOverlay.superview
            addConstraint:[NSLayoutConstraint constraintWithItem:_calibrationOverlay attribute:NSLayoutAttributeCenterX
                                                       relatedBy:NSLayoutRelationEqual
        ...
        {
            [_calibrationOverlay.superview
                addConstraint:[NSLayoutConstraint constraintWithItem:_calibrationOverlay
...
- (IBAction)panGesture:(UIPanGestureRecognizer*)gestureRecognizer
{
    if (_slamState.scannerState == ScannerStateCubePlacement)
    {
        if ([gestureRecognizer state] == UIGestureRecognizerStateBegan)
            _initialBoxDistance = self->_options.cubeDistanceValue;
        else if ([gestureRecognizer state] == UIGestureRecognizerStateChanged)
        {
            static constexpr float minDist = 0.5;
            static constexpr float maxDist = 2.5;
            const float translation =
                -[gestureRecognizer translationInView:self.view].y / self.view.frame.size.height * (maxDist - minDist);
            self->_options.cubeDistanceValue = keepInRange(_initialBoxDistance + translation, minDist, maxDist);
            self.boxDistanceLabel.text =
                [NSString stringWithFormat:@"Distance %1.2f m", self->_options.cubeDistanceValue];
        }
    }
}
        ...
        if (_slamState.scannerState == ScannerStateCubePlacement)
        {
            ...
            if (!isnan(_volumeScale.initialPinchScale))
            {
                ...
                GLKVector3 newVolumeSize =
                    GLKVector3MultiplyScalar(_options.initVolumeSizeInMeters, _volumeScale.currentScale);
...
- (void)backgroundTask:(STBackgroundTask*)sender didUpdateProgress:(double)progress
{
    if (sender == _naiveColorizeTask)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_meshViewController
                showMeshViewerMessage:[NSString stringWithFormat:@"Processing: % 3d%%", int(progress * 20)]];
        });
    }
    else if (sender == _enhancedColorizeTask)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_meshViewController
                showMeshViewerMessage:[NSString stringWithFormat:@"Processing: % 3d%%", int(progress * 80) + 20]];
        });
    }
}

- (BOOL)meshViewDidRequestColorizing:(STMesh*)mesh
            previewCompletionHandler:(void (^)())previewCompletionHandler
           enhancedCompletionHandler:(void (^)())enhancedCompletionHandler
{
    ...
    _naiveColorizeTask =
        [STColorizer newColorizeTaskWithMesh:mesh scene:_slamState.scene
                           ...
                           completionHandler:^(NSError* error) {
                               ...
                               else
                               {
                                   dispatch_async(dispatch_get_main_queue(), ^{
                                       ...
                                       [self performEnhancedColorize:(STMesh*)mesh
                                           enhancedCompletionHandler:enhancedCompletionHandler];
                                   });
                                   self->_naiveColorizeTask = nil;
                               }
                           }
                                     options:@{
                                         kSTColorizerTypeKey: @(STColorizerPerVertex),
...
- (void)performEnhancedColorize:(STMesh*)mesh enhancedCompletionHandler:(void (^)())enhancedCompletionHandler
{
    _enhancedColorizeTask = [STColorizer
        newColorizeTaskWithMesh:mesh
                      ...
                      keyframes:[_slamState.keyFrameManager getKeyFrames] completionHandler:^(NSError* error) {
                          if (error != nil)
                          {
                              NSLog(@"Error during colorizing: %@", [error localizedDescription]);
                          }
            ...
            if (_enhancedColorizeTask != nil && !_slamState.showingMemoryWarning)
            {
                ...
                UIAlertAction* okAction = [UIAlertAction
                    actionWithTitle:@"OK"
                            ...
                            handler:^(UIAlertAction* action) { self->_slamState.showingMemoryWarning = false; }];
        ...
        case ScannerStateScanning:
        {
            if (!_slamState.showingMemoryWarning)
            {
                ...
                UIAlertController* alertCtrl =
                    [UIAlertController alertControllerWithTitle:@"Memory Low"
...
- (void)batteryLevelCheckTick:(NSTimer*)timer
{
    const int batteryLevel = self->_captureSession.sensorBatteryLevel;
    BatteryLevelState currentBatteryLevelState = batteryLevel > 50 ? BatteryLevelState::Full
    ...
    if (currentBatteryLevelState != self.lastBatteryLevelState)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                image = [UIImage imageNamed:@"icon-battery-full.png"];
            }
            else if (self.lastBatteryLevelState == BatteryLevelState::Medium)
            {
                image = [UIImage imageNamed:@"icon-battery-half.png"];
            }
            else if (self.lastBatteryLevelState == BatteryLevelState::Low)
            {
                image = [UIImage imageNamed:@"icon-battery-low.png"];
            }
            else
            {
              self.batteryView.hidden = YES;
            }
            [self.batteryImageView setImage:image];
