...
- (void)setupGL
{
    ...
    CVReturn texError =
        CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, _display.context, NULL, &_display.videoTextureCache);
    if (texError)
    {
...
- (void)uploadGLColorTextureFromDepth:(STDepthFrame*)depthFrame
{
    ...
    glTexImage2D(
        ...
        0,
        GL_RGBA,
        _depthAsRgbaVisualizer.width,
        _depthAsRgbaVisualizer.height,
        0,
        GL_RGBA,
        GL_UNSIGNED_BYTE,
...
- (void)renderSceneForDepthFrame:(STDepthFrame*)depthFrame colorFrameOrNil:(STColorFrame*)colorFrame
{
    ...
    switch (_slamState.scannerState)
    {
        case ScannerStateCubePlacement:
        {
            ...
            if (_slamState.cameraPoseInitializer.lastOutput.hasValidPose)
            {
                ...
                if (_useColorCamera && !_options.useHardwareRegisteredDepth)
                {
                    ...
                    {
                        depthCameraPose = GLKMatrix4Multiply(
                            _slamState.cameraPoseInitializer.lastOutput.cameraPose,
                            iOSColorFromDepthExtrinsics);
                        ...
                        cameraViewpoint = GLKMatrix4Translate(cameraViewpoint, 0, 0, -_options.cubeDistanceValue);
                        _slamState.cameraPose = GLKMatrix4Multiply(cameraViewpoint, iOSColorFromDepthExtrinsics);
                    }
                    else
                    {
                        // colorCameraPoseInWorld
                ...
                [_display.cubeRenderer renderCubeOutlineWithCameraPose:cameraViewpoint depthTestEnabled:false
        ...
        case ScannerStateScanning:
        {
            ...
            GLKMatrix4 depthCameraPose = GLKMatrix4Identity;
            if (_slamState.slamOption == SlamOption::STSlamManager)
                depthCameraPose = [_slamState.lssTracker lastFrameCameraPose];
            else
                depthCameraPose = [_slamState.tracker lastFrameCameraPose];
            ...
            [_slamState.scene renderMeshFromViewpoint:cameraViewpoint cameraGLProjection:cameraGLProjection
            ...
            [_display.cubeRenderer renderCubeOutlineWithCameraPose:cameraViewpoint depthTestEnabled:true
...
- (void)renderCameraImage
{
    if (_useColorCamera)
    {
        ...
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(
            CVOpenGLESTextureGetTarget(_display.chromaTexture),
